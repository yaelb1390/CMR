...
export async function GET(request) {
  try {
    ...
    if (dias_min_str) {
      ...
      if (!isNaN(dias_min)) {
        // Dynamic calculation of overdue days using fecha_proximo_pago
        whereClause.push(`GREATEST(0, DATE_PART('day', CURRENT_DATE - p.fecha_proximo_pago))::int >= $${paramIndex}`);
    ...
    const sql = `
      SELECT 
        ...
        GREATEST(0, DATE_PART('day', CURRENT_DATE - p.fecha_proximo_pago))::int AS dias_atraso_calc
      FROM prestamos p
...
export async function POST(request) {
  try {
    ...
    const interes = parseFloat(tasa_interes) || 0.05; // Default 5% si no viene

    // Generate Loan Number
    const currentYear = new Date().getFullYear();
    const yearPrefix = `PBM-${currentYear}-`;
    const lastLoanRes = await query("SELECT numero_prestamo FROM prestamos WHERE numero_prestamo LIKE $1 ORDER BY numero_prestamo DESC LIMIT 1", [`${yearPrefix}%`]);
    ...
    await query("UPDATE clientes SET total_prestamos = total_prestamos + 1, capital_prestado = capital_prestado + $1 WHERE cedula = $2", [monto, cedula]);
    ...
    return NextResponse.json({ success: true, message: "Préstamo creado correctamente.", data: { numero_prestamo: generatedLoanNumber } });
  } catch (err) {
